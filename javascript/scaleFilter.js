...
function msg_float (rawPitch) {
	...
	if (rawPitch < 0.0) { rawPitch = 0; } // otherwise it can crash
	...
	if (scale.length > 3) { // if we don't have a scale, don't do this over and over
		...
		distance[0] = (rawPitch % 12.0) - scale[scaleDegreeFloor];
		distance[1] = scale[scaleDegreeFloor + 1] - (rawPitch % 12.0);
		...
		result += Math.floor(rawPitch / 12.0) * 12; // re-add octave base
		result -= keyOffset; // remove key offset
		// post("result " + result + "\n");
		outlet(0, result);
	} else if (scale.length === 1) {
		result = Math.floor(rawPitch / 12.0 + 0.5) * 12 + scale[0]; // set to octave base + scale degree of single note scale
		result -= keyOffset; // remove key offset
		outlet(0, result);
	} else {
