 ...
 * ie, DC offset
 *
 * Modified by Arvid Tomayko-Peters 2012 - 2015
...
public class Op extends MaxObject {
	...
	public void dewrapflac(final int channel, final float lowerLimit) {
		//Added by Arvid 2015-05-24
		//try to remove wraparound error for flacs saved by max with overloads
		MaxSystem.deferLow(new Executable() {
			//do it in another thread
			public void execute() {
				float[] samps = MSPBuffer.peek(bufname, channel);
				for (int i=1;i<samps.length;i++) {
					if (samps[i] <= lowerLimit) {
						if (samps[i-1] >= 0.0) {
							samps[i] = (float)1.0;
						}
					}
	...
	public void findSoundEnd() {
		//Added by Arvid 2015-07-23
		//find the last sample that has audible audio and output its position in samples
		MaxSystem.deferLow(new Executable() {
			//do it in another thread
			public void execute() {
				int i,j;
				...
				for (i=1; i<=samps.length; i+=channels) { //for each frame
					for (j=0; j<channels ;j++ ) { //for each channel
						if ((samps[samps.length-(i+j)] > 0.000001) || (samps[samps.length-(i+j)] < -0.000001)) {
							//if not very close to 0
							isSilent = false;
						}
					}
					if (!isSilent) {
				...
				//output "soundEnd sampleNum" as a list
				outlet(0, new Atom[] {Atom.newAtom("soundEnd"), Atom.newAtom((samps.length-i)/channels)});
			}
		} //end Executable
		); //end deferLow
	}
	...
	public void bufferMultiply(int channel, String otherBuffer ) {
		//Added by Arvid 2012-03-12
		//this buffer * another
		float[] samps = MSPBuffer.peek(bufname, channel);
		float[] otherSamps =  MSPBuffer.peek(otherBuffer, channel);
		int multLength = 0;
		if (samps.length > otherSamps.length) {
			multLength = otherSamps.length;
		} else {
			multLength = samps.length;
		}
		for (int i=0;i<multLength;i++)
			samps[i] *= otherSamps[i];
		MSPBuffer.poke(bufname, channel, samps);
	}

	public void bufferDiv(int channel, String otherBuffer ) {
		//Added by Arvid 2012-03-12
		...
		for (int i=0;i<multLength;i++)
			samps[i] = samps[i]/otherSamps[i];
		MSPBuffer.poke(bufname, channel, samps);
	}

	public void bufferVid(int channel, String otherBuffer ) {
		//Added by Arvid 2012-03-12
		//this buffer = other buffer / this one
		float[] samps = MSPBuffer.peek(bufname, channel);
		float[] otherSamps =  MSPBuffer.peek(otherBuffer, channel);
		int multLength = 0;
		if (samps.length > otherSamps.length) {
	...
	public void bufferAdd(int channel, String otherBuffer ) {
		//Added by Arvid 2012-03-12
		float[] samps = MSPBuffer.peek(bufname, channel);
		float[] otherSamps =  MSPBuffer.peek(otherBuffer, channel);
		int multLength = 0;
	...
	public void bufferSub(int channel, String otherBuffer ) {
		//Added by Arvid 2012-03-12
		//this buffer - another
		float[] samps = MSPBuffer.peek(bufname, channel);
		float[] otherSamps =  MSPBuffer.peek(otherBuffer, channel);
	...
	public void bufferBus(int channel, String otherBuffer ) {
		//Added by Arvid 2012-03-12
		//this buffer = other buffer - this one
		float[] samps = MSPBuffer.peek(bufname, channel);
		float[] otherSamps =  MSPBuffer.peek(otherBuffer, channel);
